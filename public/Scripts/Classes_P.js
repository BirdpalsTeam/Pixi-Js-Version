class Player extends PIXI.Sprite {
  constructor(player) {
    ...
    //ID
    this.id == sessionStorage.playerId
      ? (this.local = true)
      : (this.local = false);
    //card
    this.bio = player.bio;
  ...
  whereToLook() {
    ...
    if (angleToLook > 70 && angleToLook <= 110) {
      //look to the front
      this.lookingInt = 4;
    } else if (angleToLook > 110 && angleToLook <= 220) {
      //look to the left
      this.lookingInt = 6;
    } else if (angleToLook > 220 && angleToLook <= 260) {
      //look to the upper left
      this.lookingInt = 2;
    } else if (angleToLook > 260 && angleToLook <= 281) {
      //look to the back
      this.lookingInt = 1;
    } else if (angleToLook > 281 && angleToLook <= 330) {
      //look to the upper right
      this.lookingInt = 5;
    } else if ((angleToLook > 330 && angleToLook <= 360) || angleToLook <= 70) {
      //look to the right
      this.lookingInt = 3;
    }
  ...
  movePlayer() {
    ...
    this.movePlayerInterval.add((delta) => {
      ...
      if (collisions == collisionArray.length) {
        ...
        triggers.forEach(function (tempTrigger) {
          //Goes through each trigger to see if the player is within it
          if (
            localPlayer.y >= tempTrigger[1] &&
            localPlayer.y <= tempTrigger[3] &&
            localPlayer.x >= tempTrigger[0] &&
            localPlayer.x <= tempTrigger[2]
          ) {
    ...
    function littleJump(currentDuration) {
      ...
      if (currentDuration > 20) {
        ...
        y =
          Math.abs(0.3 * Math.sin(((2 * Math.PI) / 40) * currentDuration)) +
          0.95;
      } else if (currentDuration <= 20 || currentDuration == rest) {
        //Vertical launch equation
        //y = 0.1 * x - 5 / 2
        y =
          1 / (0.1 * currentDuration - 5 * Math.pow(currentDuration, 2)) + 1.15;
      }
      return y;
    }
        ...
        gearImgs.forEach((item) => {
          if (currentDuration === true) {
            ...
            if (currentDuration > 20) {
              ...
              item.anchor.y =
                Math.abs(
                  1.1 * Math.sin(((2 * Math.PI) / 40) * currentDuration)
                ) + 0.5;
            } else if (currentDuration <= 20 || currentDuration == rest) {
              //Vertical launch equation
              //y = 0.1 * x - 5 / 2
              item.anchor.y =
                1 / (0.1 * currentDuration - 5 * Math.pow(currentDuration, 2)) +
                0.7;
            }
          }
        });
  ...
  drawBubble() {
    if (this.message != undefined) {
      ...
      if (this.bitmapText.width > 100) {
        this.bubble.width = this.bitmapText.width + 20;
      } else {
        this.bubble.width = 100;
      }
      if (this.bitmapText.height > 30) {
        this.bubble.height = this.bitmapText.height + 20;
      } else {
        this.bubble.height = 35;
      }
      ...
      PIXI.sound.play('bubblePop', { volume: 0.25 });
